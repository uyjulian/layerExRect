...
TVP_GL_FUNC_DECL(void, TVPRemoveConstOpacity_c, (tjs_uint32 *dest, tjs_int len, tjs_int strength))
{
	...
	strength = 255u - strength;

		{
			...
			{
	d = dest[___index];;
	dest[___index] = (d & 0xffffffu) + ( (((d>>24u)*strength) << 16u) & 0xff000000u);;
			}
		}
}
...
TVP_GL_FUNC_DECL(void, TVPConstColorAlphaBlend_c, (tjs_uint32 *dest, tjs_int len, tjs_uint32 color, tjs_int opa))
{
	...
	{
		v4  = dest;
		v5  = simde_mm_cvtsi32_si64(0xFF000000);
		v6  = simde_m_punpckldq(v5, v5);
		v7  = simde_mm_cvtsi32_si64(0xFFFFFFu);
		v8  = simde_m_punpckldq(v7, v7);
		v10 = simde_mm_set1_pi16(opa);
		v11 = simde_m_pmullw(simde_m_punpcklbw(simde_mm_cvtsi32_si64(color), simde_mm_setzero_si64()), v10);
		v13 = simde_mm_set1_pi16((tjs_uint8) ~(tjs_uint8)opa);
		v14 = v13;
		v15 = &dest[len];
		if (dest < v15)
		{
			do
			{
				v16 = simde_mm_cvtsi32_si64(*v4);
				*v4 = simde_mm_cvtsi64_si32(
					simde_m_por(
						simde_m_pand(
							simde_m_packuswb(simde_m_psrlwi(simde_m_paddusw(simde_m_pmullw(simde_m_punpcklbw(v16, simde_mm_setzero_si64()), v14), v11), 8u), simde_mm_setzero_si64()),
							v8),
						simde_m_pand(v16, v6)));
				++v4;
			} while (v4 < v15);
...
TVP_GL_FUNC_DECL(void, TVPConstColorAlphaBlend_d_c, (tjs_uint32 *dest, tjs_int len, tjs_uint32 color, tjs_int opa))
{
	...
	if (len > 0)
	{
		...
		if (dest < v6)
		{
			do
			{
				v9         = simde_m_punpcklbw(simde_mm_cvtsi32_si64(*v5 & 0xFFFFFF), simde_mm_setzero_si64());
				v10        = *v5 >> 24;
				LOBYTE(v4) = TVPNegativeMulTable[v8 + v10];
				v4 <<= 24;
				v12 = simde_mm_set1_pi16(TVPOpacityOnOpacityTable[v8 + v10]);
				*v5 = v4 | simde_mm_cvtsi64_si32(simde_m_packuswb(simde_m_psrlwi(simde_m_paddw(simde_m_pmullw(simde_m_psubw(v7, v9), v12), simde_m_psllwi(v9, 8u)), 8u), simde_mm_setzero_si64()));
				++v5;
			} while (v5 < v6);
		}
	}
...
TVP_GL_FUNC_DECL(void, TVPConstColorAlphaBlend_a_c, (tjs_uint32 *dest, tjs_int len, tjs_uint32 color, tjs_int opa))
{
	...
	if (len > 0)
	{
		...
		v8  = simde_mm_set1_pi16(v6);
		v9  = v8;
		v10 = simde_m_por(
			simde_m_psrlwi(simde_m_pmullw(simde_m_punpcklbw(simde_mm_cvtsi32_si64(color & 0xFFFFFF), simde_mm_setzero_si64()), v9), 8u),
			simde_m_punpcklbw(simde_mm_cvtsi32_si64(v6 << 24), simde_mm_setzero_si64()));
		if (dest < v5)
		{
			do
			{
				v11 = simde_m_punpcklbw(simde_mm_cvtsi32_si64(*v4), simde_mm_setzero_si64());
				*v4 = simde_mm_cvtsi64_si32(simde_m_packuswb(simde_m_paddw(simde_m_psubw(v11, simde_m_psrlwi(simde_m_pmullw(v11, v9), 8u)), v10), simde_mm_setzero_si64()));
				++v4;
			} while (v4 < v5);
		}
	}
...
TVP_GL_FUNC_DECL(void, TVPCopyMask_c, (tjs_uint32 *dest, const tjs_uint32 *src, tjs_int len))
{
	if(dest < src)
	{
		...
		{
		{
			dest[___index] = (dest[___index] & 0xffffffu) +
				(src[___index] & 0xff000000u);
		}
		}
	}
	else
